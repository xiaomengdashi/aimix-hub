...
export const AccountArchivedList: FC<AccountArchivedListProps> = ({
  ...
  const handleUnarchive = async (threadId: string) => {
    ...
    try {
      ...
      setLocalThreads((current) => current.filter((thread) => thread.id !== threadId));
      router.refresh();
    } finally {
      setPendingId(null);
    }
  ...
  return (
    <section id="archived" className="scroll-mt-6 rounded-xl border bg-card shadow-sm">
      <div className="border-b px-5 py-4">
        <h2 className="font-medium text-base">已归档会话</h2>
        <p className="text-muted-foreground text-sm">
          {localThreads.length > 0
            ? `共 ${localThreads.length} 个会话，恢复后将重新出现在侧边栏`
            : "归档的会话会保存在云端，可随时在此恢复"}
        </p>
      </div>
